...
public final class CancelTokenSource implements ICancelTokenSource {
     ...
     */
    @Override
    public void cancelAfter(int cancelCode, long time, TimeUnit timeUnit) {
        cancelAfter(cancelCode, time, timeUnit, delayer);
    }
     ...
     */
    @Override
    public void cancelAfter(int cancelCode, long time, TimeUnit timeUnit, ScheduledExecutorService executorService) {
        Objects.requireNonNull(executorService, "executorService");
        if (this.code == 0) {
            Canceller canceller = new Canceller(this, cancelCode);
            canceller.future = executorService.schedule(canceller, time, timeUnit);
            register(canceller); // 及时取消定时任务
        }
    }
