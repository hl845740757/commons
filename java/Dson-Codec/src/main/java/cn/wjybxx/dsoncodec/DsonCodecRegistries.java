...
public class DsonCodecRegistries {
    ...
    public static DsonCodecRegistry fromRegistries(List<? extends DsonCodecRegistry> codecRegistries) {
        final SimpleCodecRegistry simpleCodecRegistry = new SimpleCodecRegistry();
        ...
        for (DsonCodecRegistry codecRegistry : codecRegistries) {
            if (codecRegistry instanceof SimpleCodecRegistry other) {
        ...
        // 简单Codec放在最前面
        unmergedRegistries.add(0, simpleCodecRegistry.toImmutable());
