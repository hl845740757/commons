...
public class LocalTimeCodec implements DsonCodec<LocalTime> {
    ...
    public void writeObject(DsonObjectWriter writer, LocalTime inst, TypeInfo declaredType, ObjectStyle style) {
        writer.writeExtDateTime(null, toDateTime(inst));
    }
    ...
    private static LocalTime ofDateTime(ExtDateTime dateTime) {
        long nanoOfDay = dateTime.getSeconds() * TimeUtils.NANOS_PER_SECOND + dateTime.getNanos();
        return LocalTime.ofNanoOfDay(nanoOfDay);
    }

    private static ExtDateTime toDateTime(LocalTime localTime) {
        return new ExtDateTime(localTime.toSecondOfDay(), localTime.getNano(), 0, ExtDateTime.MASK_TIME);
    }
}