...
/// <summary>
/// 通过<see cref="Type.GenericParameterAttributes"/>和<see cref="GenericParameterAttributes"/>来测试泛型信息，
...
public class GenericTypeTest2
{
    ...
    /// 注意：<see cref="ValueType"/>和<see cref="Void"/>都不能直接当做泛型参数。
    /// </summary>
    ...
    /// <summary>
    /// 输出为：genericArgument: T, constraints: System.Type[], attributes: ReferenceTypeConstraint
    /// </summary>
    ...
    /// <summary>
    /// 输出为：genericArgument: T, constraints: [Wjybxx.Dson.DsonValue], attributes: None
    /// </summary>
    private class GenericTypeC<T> where T : DsonValue
    {
    }
    ...
    /// <summary>
    /// 输出为：genericArgument: T, constraints: [Wjybxx.Dson.DsonValue], attributes: DefaultConstructorConstraint
    ///
    /// 可见：notnull关键字并没有实际的作用，而且不能和struct、class关键字同时使用。
    /// </summary>
    private class GenericTypeE<T> where T : notnull, DsonValue?, new()
    {
    }
