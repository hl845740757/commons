...
public class TypeVariableName : TypeName
{
    ...
    public override TypeVariableName WithAttributes(TypeNameAttributes attributes) {
#else
    ...
    private static TypeVariableName InternalGet(string name, IList<TypeName>? bounds, TypeNameAttributes attributes) {
        ...
        List<TypeName> visibleBounds = new List<TypeName>(bounds.Count);
        foreach (TypeName bound in bounds) {
            if (ReferenceEquals(bound, OBJECT)) {
                continue;
            }
            if (ReferenceEquals(bound, ClassName.VALUE_TYPE)) {
                attributes |= TypeNameAttributes.ValueTypeConstraint;
                continue;
            }
            visibleBounds.Add(bound);
        }
    ...
    public TypeVariableName AddBounds(params TypeName[] bounds) {
        if (bounds.Length == 0) return this;
        List<TypeName> newBounds = new(this.bounds.Count + bounds.Length);
    ...
    /// <summary>
    /// 替换约束，需注意object和nullable
    /// </summary>
    /// <param name="bounds">边界</param>
    /// <param name="attributes">属性</param>
    /// <returns>新的对象</returns>
    public TypeVariableName WithBounds(IList<TypeName> bounds, TypeNameAttributes attributes = TypeNameAttributes.None) {
        return new TypeVariableName(name, bounds, attributes);
    }
    ...
    /// 约束是否为值类型
    /// </summary>
    public bool IsValueTypeConstraint => (attributes & TypeNameAttributes.ValueTypeConstraint) != 0;
    ...
    public string ConstraintsToString() {
        ...
        if ((attributes & TypeNameAttributes.ValueTypeConstraint) != 0) {
            return "struct";
        }
        ...
        if ((attributes & TypeNameAttributes.ReferenceTypeConstraint) != 0) {
            sb.Append("class");
            if ((attributes & TypeNameAttributes.NullableReferenceType) != 0) {
                sb.Append('?');
            }
            count++;
        }
