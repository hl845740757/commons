    ...
    private static TypeSpec BuildClassType() {
        ...
        TypeSpec classType = TypeSpec.NewClassBuilder("ClassBean")
            ...
            .AddSpec(FieldSpec.NewBuilder(dictionaryTypeName, "blackboard", Modifiers.Public | Modifiers.ReadOnly)
                ...
                .Build())
            // 事件
            .AddSpec(EventSpec.NewBuilder(TypeName.Get(typeof(EventHandler)), "SimpleChanged", Modifiers.Public)
                .AddDocument("字段式事件")
                .Build())
            .AddField(TypeName.Get(typeof(EventHandler)), "changed", Modifiers.Private)
            .AddSpec(EventSpec.NewBuilder(TypeName.Get(typeof(EventHandler)), "Changed", Modifiers.Public)
                .AddDocument("表达式风格的自定义访问器事件")
                .Adder(CodeBlock.Of("changed += value").WithExpressionStyle(true))
                .Remover(CodeBlock.Of("changed -= value").WithExpressionStyle(true))
                .Build())
            .AddSpec(EventSpec.NewBuilder(TypeName.Get(typeof(EventHandler)), "Changed2", Modifiers.Public)
                .AddDocument("块风格的自定义访问器事件")
                .Adder(CodeBlock.NewBuilder()
                    .AddStatement("changed += value")
                    .Build())
                .Remover(CodeBlock.NewBuilder()
                    .AddStatement("changed -= value")
                    .Build())
                .Build())
            // 构造函数
