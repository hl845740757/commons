...
public interface IArrayPool<T> : IObjectPool<T[]>
{
    ...
    /// 2. 默认不执行清理(稳定API)
    /// </summary>
    public static IArrayPool<T> Shared { get; } = new ConcurrentArrayPool<T>(4096, 1024 * 1024);
    ...
    /// 2. 默认不执行清理(稳定API)
    /// </summary>
    public static IArrayPool<T> SystemShared { get; } = new ConcurrentArrayPool<T>(ArrayPool<T>.Shared, 4096);
