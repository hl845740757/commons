...
public sealed class Join1Codec<T> : AbstractDsonCodec<Join<T>> where T : class
{
    ...
    protected override void WriteFields(IDsonObjectWriter writer, ref Join<T> inst, Type declaredType, ObjectStyle style) {
        ...
        writer.WriteObject(names_policy, inst.Policy, typeof(JoinPolicy<T>), null);
    }

    protected override Join<T> NewInstance(IDsonObjectReader reader, Type declaredType) {
        return new Join<T>();
    }

    protected override void ReadFields(IDsonObjectReader reader, ref Join<T> inst, Type declaredType) {
