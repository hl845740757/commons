...
public sealed class OnlyOnce1Codec<T> : AbstractDsonCodec<OnlyOnce<T>> where T : class
{
    ...
    protected override OnlyOnce<T> NewInstance(IDsonObjectReader reader, Type declaredType) {
        return new OnlyOnce<T>();
    }

    protected override void ReadFields(IDsonObjectReader reader, ref OnlyOnce<T> inst, Type declaredType) {
