...
public sealed class FixedSwitch1Codec<T> : AbstractDsonCodec<FixedSwitch<T>> where T : class
{
    ...
    protected override void ReadFields(IDsonObjectReader reader, ref FixedSwitch<T> inst) {
        ...
        if (reader.ReadName(names_handler)) inst.Handler = reader.ReadObject<ISwitchHandler<T>>(null, typeof(ISwitchHandler<T>), null);
        if (reader.ReadName(names_branch1)) inst.Branch1 = reader.ReadObject<Task<T>>(null, typeof(Task<T>), null);
        if (reader.ReadName(names_branch2)) inst.Branch2 = reader.ReadObject<Task<T>>(null, typeof(Task<T>), null);
        if (reader.ReadName(names_branch3)) inst.Branch3 = reader.ReadObject<Task<T>>(null, typeof(Task<T>), null);
        if (reader.ReadName(names_branch4)) inst.Branch4 = reader.ReadObject<Task<T>>(null, typeof(Task<T>), null);
        if (reader.ReadName(names_branch5)) inst.Branch5 = reader.ReadObject<Task<T>>(null, typeof(Task<T>), null);
    }
}
}