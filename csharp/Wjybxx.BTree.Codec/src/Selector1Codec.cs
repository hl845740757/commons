...
public sealed class Selector1Codec<T> : AbstractDsonCodec<Selector<T>> where T : class
{
    ...
    public override Type GetEncoderClass() => typeof(Selector<T>);

    protected override void WriteFields(IDsonObjectWriter writer, ref Selector<T> inst, Type declaredType, ObjectStyle style) {
    ...
    protected override Selector<T> NewInstance(IDsonObjectReader reader, Type declaredType) {
        return new Selector<T>();
    }

    protected override void ReadFields(IDsonObjectReader reader, ref Selector<T> inst, Type declaredType) {
