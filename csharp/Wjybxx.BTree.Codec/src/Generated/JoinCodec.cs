...
public sealed class JoinCodec<T> : AbstractDsonCodec<Join<T>> where T : class 
{
    ...
    protected override void WriteFields(IDsonObjectWriter writer, in Join<T> inst) {
        ...
        writer.WriteObject(names_policy, inst.Policy, null);
    }

    protected override Join<T> NewInstance(IDsonObjectReader reader) {
        return new Join<T>();
    }
    ...
    protected override void ReadFields(IDsonObjectReader reader, ref Join<T> inst) {
        if (reader.ContextType == DsonContextType.Array) {
            ...
            inst.Policy = reader.ReadObject<JoinPolicy<T>>(null, null);
            return;
        }
        while (reader.ReadDsonType() != DsonType.EndOfObject) {
            switch (reader.ReadName()) {
                ...
                case names_policy: inst.Policy = reader.ReadObject<JoinPolicy<T>>(null, null); break;
            }
        }
    }
}
