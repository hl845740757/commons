...
public sealed class Inverter1Codec<T> : AbstractDsonCodec<Inverter<T>> where T : class
{
    ...
    protected override void WriteFields(IDsonObjectWriter writer, ref Inverter<T> inst) {
    ...
    protected override Inverter<T> NewInstance(IDsonObjectReader reader) {
        return new Inverter<T>();
    }

    protected override void ReadFields(IDsonObjectReader reader, ref Inverter<T> inst) {
