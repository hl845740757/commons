...
public sealed class Inverter1Codec<T> : AbstractDsonCodec<Inverter<T>> where T : class
{
    ...
    protected override Inverter<T> NewInstance(IDsonObjectReader reader, Type declaredType) {
        return new Inverter<T>();
    }

    protected override void ReadFields(IDsonObjectReader reader, ref Inverter<T> inst, Type declaredType) {
