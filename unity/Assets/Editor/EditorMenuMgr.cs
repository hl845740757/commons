...
namespace Wjybxx.Commons.Editor
{
/// <summary>
/// 用于拷贝Csharp代码到Unity下
///
/// 注意：
/// 1.由于Roslyn的代码生成器生成的文件路径不稳定，如果执行了Clean，应当Rebuild两次，可保证生成的文件不遗留。
/// 2.校验是否是需要删除的文件时，不校验顶层目录的文件。
/// </summary>
public class EditorMenuMgr
{
    /// <summary>
    /// 需要拷贝的普通程序集
    /// </summary>
    private static readonly Dictionary<string, bool> _projectNames = new()
    {
        { "Wjybxx.Commons.Core", false },
        { "Wjybxx.Commons.Inject", false },
        { "Wjybxx.Commons.Concurrent", false },
        { "Wjybxx.Disruptor", false },
        { "Wjybxx.Dson.Core", false },
        { "Wjybxx.Dson.Codec", false },
        { "Wjybxx.BTree.Core", false },
        { "Wjybxx.BTree.Codec", false },
        { "Wjybxx.Logger.Unity", false },

        { "Wjybxx.Commons.Poet", true },
        { "Wjybxx.Dson.Apt2", true }, // 为Unity库生成Codec
    };

    /// <summary>
    /// 需要拷贝的文件
    /// (Generated目录统一在程序集的顶层目录)
    /// Unity下无法简单做到和普通Dotnet项目那样执行Apt...所以我们还是会拷贝DsonCodec生成的代码
    /// </summary>
    ...
    /// 
    /// </summary>
    [MenuItem("Editor/CreateUpkg")]
    public static void CreateUpkg() {
        foreach (var pair in _projectNames) {
            string projectName = pair.Key;
            bool editor = pair.Value;
            CopyProject(projectName, editor);
        }
    ...
    /// <param name="editor">是否是editor项目</param>
    private static void CopyProject(string projectName, bool editor) {
        // dataPath为Assets目录
        DirectoryInfo rootDir = new DirectoryInfo(Application.dataPath)
            .Parent!.Parent!;
        string srcProjectDir = rootDir.FullName + "/csharp/" + projectName;
        string destProjectDir = Application.dataPath + "/Scripts/" + projectName;
        ...
        foreach (string srcDir in _includes) {
            ...
            string midPath = editor ? "Editor" : "Runtime";
            string destSubDir = srcDir == "src"
                ? destProjectDir + "/" + midPath
                : destProjectDir + "/" + midPath + "/" + srcDir;
